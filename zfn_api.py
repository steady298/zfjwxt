...
class Client:
    ...
    def __init__(self, cookies={}, **kwargs):
        ...
        self.key_url = urljoin(self.base_url, "http://59.74.174.150/jwglxt/xtgl/login_getPublicKey.html")
        self.login_url = urljoin(self.base_url, "http://59.74.174.150/jwglxt/xtgl/login_slogin.html")
        self.kaptcha_url = urljoin(self.base_url, "kaptcha")
        self.headers = requests.utils.default_headers()
        self.headers["Referer"] = self.login_url
        self.headers[
            "User-Agent"
        ] = "Mozilla/5.0 (Windows NT 6.2; rw-RW; rv:1.9.0.20) Gecko/2023-02-06 17:38:30 Firefox/3.6.10"
        self.headers[
            "Accept"
    ...
    def get_info(self):
        """获取个人信息"""
        url = urljoin(self.base_url, "http://59.74.174.150/jwglxt/xsxxxggl/xsxxwh_cxCkDgxsxx.html?gnmkdm=N100801")
        try:
    ...
    def _get_info(self):
        """获取个人信息"""
        url = urljoin(self.base_url, "http://59.74.174.150/jwglxt/xsxxxggl/xsgrxxwh_cxXsgrxx.html?gnmkdm=N100801&layout=default")
        try:
            req_info = self.sess.get(
    ...
    def get_grade(self, year: int, term: int = 0, use_personal_info: bool = False):
        ...
        url = urljoin(
            self.base_url,
            "http://59.74.174.150/jwglxt/cjcx/cjcx_cxDgXscj.html?doType=query&gnmkdm=N305005"
            if use_personal_info
            else "http://59.74.174.150/jwglxt/cjcx/cjcx_cxXsgrcj.html?doType=query&gnmkdm=N305005",
        )
        temp_term = term
        term = term**2 * 3
    ...
    def get_academia(self):
        ...
        url_main = urljoin(
            self.base_url,
            "http://59.74.174.150/jwglxt/xsxy/xsxyqk_cxXsxyqkIndex.html?gnmkdm=N105515&layout=default",
        )
        url_info = urljoin(
            self.base_url, "http://59.74.174.150/jwglxt/xsxy/xsxyqk_cxJxzxjhxfyqKcxx.html?gnmkdm=N105515"
        )
        try:
